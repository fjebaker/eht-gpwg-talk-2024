...
begin
    fig = Figure(resolution = (430, 400))
    ga = fig[1, 1] = GridLayout()
    ax = Axis(ga[1, 1], ylabel = L"|Deflection| $/\pi$", title = "Deflection angle")
    ylims!(ax, 0.0, 2.0)

    vspan!(ax, [-shadow(m; pos = false)], [-shadow(m; pos = true)],
    ...
    vspan!(ax2, [-shadow(m; pos = false)], [-shadow(m; pos = true)],
        color = RGBAf(0.1, 0.2, 0.3, 0.1),
        strokewidth = 1.0,
    )
    ...
    linkxaxes!(ax2, ax)
    rowsize!(ga, 1, Auto(1.5))
    rowgap!(ga, 15)
    xlims!(ax2, -20, 20)

    resize_to_layout!(fig)
    Makie.save("presentation/figs/raw/deflection-angle.svg", fig)
    fig
end
