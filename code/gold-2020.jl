...
begin
    ...
    ax1 = Axis(ga[1, 1], aspect = DataAspect(), ylabel = L"\beta", xlabel = L"\alpha", backgroundcolor = RGBAf(0.0,0.0,0.0,0.0))
    hm1 = plot_heatmap!(ax1, a1, b1, im1)
    crange = (0.0, 2e-4)
    Label(ga[1, 1, Top()], "Test 1", padding = (0, 0, 0, 0), font = :bold)

    ax2 = Axis(ga[1, 2], aspect = DataAspect(), xlabel = L"\alpha", backgroundcolor = RGBAf(0.0,0.0,0.0,0.0))
    hm2 = plot_heatmap!(ax2, a2, b2, im2, colorrange = crange)
    Label(ga[1, 2, Top()], "Test 2", padding = (0, 0, 0, 0), font = :bold)

    ax3 = Axis(ga[1, 3], aspect = DataAspect(), xlabel = L"\alpha", backgroundcolor = RGBAf(0.0,0.0,0.0,0.0))
    hm3 = plot_heatmap!(ax3, a3, b3, im3, colorrange = crange)
    Label(ga[1, 3, Top()], "Test 3", padding = (0, 0, 0, 0), font = :bold)

    ax4 = Axis(ga[1, 4], aspect = DataAspect(), xlabel = L"\alpha", backgroundcolor = RGBAf(0.0,0.0,0.0,0.0))
    hm4 = plot_heatmap!(ax4, a4, b4, im4, colorrange = crange)
    Label(ga[1, 4, Top()], "Test 4", padding = (0, 0, 0, 0), font = :bold)

    ax5 = Axis(ga[1, 5], aspect = DataAspect(), xlabel = L"\alpha", backgroundcolor = RGBAf(0.0,0.0,0.0,0.0))
    ...
    Makie.save("presentation/figs/raw/gold-2022.png", fig, dpi=400)
    fig
end
