...
a, b, img = rendergeodesics(m, x, ThinDisc(0.0, 14.0), 2x[2]; verbose = true, pf = ConstPointFunctions.redshift(m, x) ∘ ConstPointFunctions.filter_intersected(), callback = domain_upper_hemisphere(), αlims = (-20, 20), βlims = (-5, 12), image_width = 800, image_height = 800)

begin
    fig = Figure(size = (600,300), backgroundcolor = RGBAf(0.0, 0.0, 0.0, 0.0))
    ax = Axis(fig[1,1], aspect = DataAspect(), xlabel = "α", ylabel = "β", 
    # title = "Parameterization θ=75° a=0.998",
    ...
    # plot_parameterization!(ax, data1, radial1, minor_radial1, g✶s, palette)
    palette = _default_palette()
    
    ylims!(ax, -7, 12)
    xlims!(ax, -18, 24)
    
    Makie.save("presentation/figs/raw/ip-parameterization.png", fig)
    fig
end
